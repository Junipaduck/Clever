...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath }/resources/resources/css/talk.css">

</head>
<body>
    <!-- 채팅 아이콘 -->
    <div class="chatIcon font_jua">
        <img src="${pageContext.request.contextPath }/resources/resources/img/chat-icon.png" class="iconImg">
    </div>
                ...
                success:function(data){
                    ...
                    if(data.length > 0){
                        ...
                        for(var i in data){
                            ...
                            // 자신이 구매자 입장일 때
                            if(data[i].userNAme == "${loginUser.name}"){
                                // 현재 판매자가 로그인 상태 일 때
                                if(loginList.indexOf(data[i].masterEmail) != -1){
                                    $div = $("<div class='chatList_box enterRoomList' onclick='enterRoom(this);'>").attr("id",data[i].roomId).attr("name",data[i].masterName);
                                }
                                // 현재 판매자가 로그아웃 상태 일 때
                                else{
                                    $div = $("<div class='chatList_box2 enterRoomList' onclick='enterRoom(this);'>").attr("id",data[i].roomId).attr("name",data[i].masterName);
                                }
                            ...
                            // 자신이 판매자 입장일 때
                                ...
                                // 현재 구매자가 로그인 상태 일 때
                                if(loginList.indexOf(data[i].userName) != -1){
                                    $div = $("<div class='chatList_box enterRoomList' onclick='enterRoom(this);'>").attr("id",data[i].roomId).attr("name",data[i].userName);
                                }
                                // 현재 구매자가 로그아웃 상태 일 때
                                else{
                                    $div = $("<div class='chatList_box2 enterRoomList' onclick='enterRoom(this);'>").attr("id",data[i].roomId).attr("name",data[i].userName);
                                }
    ...
    <script>
        ...
        function CheckLR(data) {
            // email이 loginSession의 name과 다르면 왼쪽, 같으면 오른쪽
            const LR = (data.name != "${ loginUser.name }") ? "left" : "right";
             // 메세지 추가
            appendMessageTag(LR, data.name, data.message);
        }
